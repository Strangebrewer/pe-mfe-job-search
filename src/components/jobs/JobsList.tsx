...
const JobsList: FC = () => {
  ...
  const { company, recruiter, status, workFrom, dateMin, dateMax, archived, includeDeclined, sortBy, sortDir, setFilter } = useJobFilterStore();
  ...
  const queryParams: Record<string, any> | undefined = (() => {
    ...
    if (sortBy) { p.sortBy = sortBy; p.sortDir = sortDir || 'asc'; }
